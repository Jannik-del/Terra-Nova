...
class AdminMessageSelectMenu(commands.Cog):
    def __init__(self, client):
        self.client = client

    @nextcord.slash_command(name="admin_message_menu")
    @commands.has_any_role(1352254181305618495, 1352254181297094695, 1352254181297094692)
    async def admin_message_menu(self, ctx):
        # Erstellung des Dropdown-Menüs für die Kanalauswahl
        channel_options = [
            SelectOption(label="---", value="---"),
            SelectOption(label="rules", value="1352254181741957125"),
            SelectOption(label="News", value="1352254181741957126"),
            SelectOption(label="--", value="--"),
        ]
        ...
        class MessageMenuView(nextcord.ui.View):
            def __init__(self, client):
                super().__init__()
        ...
        view = MessageMenuView(client=self.client)
        await ctx.send("Wähle den Kanal und das Nachrichtenformat aus:", view=view)

def setup(client):
    client.add_cog(AdminMessageSelectMenu(client))
